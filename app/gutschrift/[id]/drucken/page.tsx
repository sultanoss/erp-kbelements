...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const gs = await prisma.invoice.findUnique({ where: { id }, select: { number: true } });
  return { title: `Gutschrift ${gs?.number ?? ""}` };
}
...
export default async function GutschriftDruckenPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  ...
  const sameMwst = shippingMwst === gs.mwstRate;
    ...
    <>
      ...
      <div className="page">
        ...
        <div className="header-bar">
          <div style={{ maxWidth: "55%" }}>
            ...
            <div className="customer-block">
              <div style={{ fontWeight: "bold" }}>{gs.customerName}</div>
              <div style={{ whiteSpace: "pre-line" }}>{gs.customerAddress}</div>
            </div>
          </div>
        ...
        <div className="clearfix">
          <div className="doc-title">Gutschrift</div>
          <div className="meta-block">
            <table>
              <tbody>
                <tr><td>Gutschrift-Nr:</td><td>{gs.number}</td></tr>
                <tr><td>Datum:</td><td>{fmtDate(gs.date)}</td></tr>
                {gs.originalInvoiceNum && <tr><td>Bezug:</td><td>{gs.originalInvoiceNum}</td></tr>}
                {gs.customerNum && <tr><td>Kunden-Nr:</td><td>{gs.customerNum}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        {gs.notes && <div className="notes-block">{gs.notes}</div>}
        ...
        <div className="totals-wrap">
          <table className="totals">
            <tbody>
              {sameMwst || shipping === 0 ? (
                <>
                  <tr><td>Gesamt Netto ({gs.mwstRate},00 %)</td><td>{fmt(totalNetto)} €</td></tr>
                  {gs.mwstRate > 0 && <tr><td>zzgl. MwSt ({gs.mwstRate},00 %)</td><td>{fmt(totalMwstAmt)} €</td></tr>}
                </>
              ) : (
                <>
              ...
              <tr className="bold grand-total"><td><strong>Gutschriftsbetrag</strong></td><td><strong>{fmt(bruttoGesamt)} €</strong></td></tr>
            </tbody>
          </table>
        </div>
      </div>
