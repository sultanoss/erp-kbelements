...
export default function WareMedia({ wareId }: { wareId: string }) {
  ...
  async function loadFiles() {
    ...
    const records: FileRecord[] = data
      ...
      .map((f) => ({
        name: f.name,
        path: `${prefix}/${f.name}`,
        mediaType: getMediaTypeFromExt(f.name),
      }));
    ...
    <div className="card p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          disabled={uploading || loading || files.length >= MAX_MEDIA}
