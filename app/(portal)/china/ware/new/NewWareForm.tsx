  ...
  function handleFileSelect(fileList: FileList | null) {
    ...
    for (const file of newFiles) {
      ...
      if (mediaType === "video" && file.size > MAX_VIDEO_BYTES) {
        setError(`Video "${file.name}" zu groß (max. 100 MB).`); continue;
      }
      if (!["image", "video"].includes(mediaType) && file.size > MAX_DOC_BYTES) {
        setError(`"${file.name}" zu groß (max. 20 MB).`); continue;
      }
    ...
    if (pendingFiles.length > 0) {
      ...
      for (let i = 0; i < pendingFiles.length; i++) {
        ...
        try {
          ...
          await supabase.storage.from(BUCKET).upload(path, uploadBlob, { contentType, upsert: false });
        } catch {
          // Fehler beim Upload einzelner Datei ignorieren, weiter machen
        }
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex justify-end gap-3">
        <button type="button" className="btn-secondary" onClick={() => router.push("/china/ware")} disabled={saving}>Abbrechen</button>
