...
export default function KalenderMedia({ eintragsId, userId }: { eintragsId: string; userId: string }) {
  ...
  useEffect(() => {
    supabase.from("kalender_media").select("id, storage_path, filename, media_type")
      .eq("eintrag_id", eintragsId).order("created_at")
      .then(({ data }) => {
        if (data?.length) {
          setMedia(data);
          Promise.all(
  ...
  async function handleFiles(files: FileList | null) {
    ...
    setError("");
    if (media.length >= MAX_FILES) { setError(`Maximal ${MAX_FILES} Dateien pro Eintrag.`); return; }
    ...
    for (const file of toUpload) {
      ...
      try {
        ...
        const path = `${eintragsId}/${randomId()}.${ext}`;
        const { error: upErr } = await supabase.storage.from(BUCKET).upload(path, uploadBlob, { contentType, upsert: false });
        if (upErr) throw upErr;

        const { data: row, error: dbErr } = await supabase.from("kalender_media")
          .insert({ eintrag_id: eintragsId, storage_path: path, filename: file.name, media_type: mediaType, user_id: userId })
  ...
  async function handleDelete(item: MediaRecord) {
    ...
    await supabase.from("kalender_media").delete().eq("id", item.id);
