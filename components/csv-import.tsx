  ...
  return (
    <div className="space-y-4">
      <form action={action} className="grid gap-4 md:grid-cols-3">
        <input type="hidden" name="source" value="TAGESVERKAUF" />
        ...
        <div className="flex items-end">
          ...
          >
            {pending ? "Wird importiert…" : "Verkäufe importieren"}
          </button>
        </div>
      </form>
      ...
      {undoState.done && (
        <div className="rounded-lg border border-grey-border bg-grey-light px-4 py-3 font-mono text-sm text-grey-dark">
          ✓ {undoState.count} Verkäufe rückgängig gemacht, Bestand wiederhergestellt.
        </div>
      )}
