...
import { importHerdsetSales, undoHerdsetImport } from "@/app/actions";
import { Field } from "@/components/ui";

const initialImport = { ok: false, imported: 0, errors: [] as string[], saleIds: [] as string[] };
const initialUndo = { done: false, count: 0 };

export function HerdsetImport() {
  const [state, action, pending] = useActionState(importHerdsetSales, initialImport);
  const [undoState, undoAction, undoPending] = useActionState(undoHerdsetImport, initialUndo);
    ...
    <div className="space-y-4">
      <form action={action} className="grid gap-4 md:grid-cols-3">
        ...
        <div className="flex items-end">
          ...
          >
            {pending ? "Wird importiert…" : "Herdsets importieren"}
          </button>
        </div>
      </form>
      ...
      {state.ok && state.imported > 0 && !undoState.done && (
        <div className="flex items-center justify-between rounded-lg border border-green-200 bg-green-50 px-4 py-3">
          <span className="font-mono text-sm text-green-700">
            ✓ {state.imported} Herdset-Eintrag{state.imported !== 1 ? "e" : ""} importiert.
          </span>
          {showUndo && (
      ...
      {undoState.done && (
        <div className="rounded-lg border border-grey-border bg-grey-light px-4 py-3 font-mono text-sm text-grey-dark">
          ✓ {undoState.count} Einträge rückgängig gemacht.
        </div>
      )}
