...
export function EbayOutletImport() {
  const [state, action, pending] = useActionState(importSalesCSV, initialImport);
  const [undoState, undoAction, undoPending] = useActionState(undoImport, initialUndo);

  const showUndo = state.ok && state.saleIds.length > 0 && !undoState.done;

  return (
    <div className="space-y-4">
      <form action={action} className="grid gap-4 md:grid-cols-3">
        <input type="hidden" name="source" value="NS_LAGER" />
        <input type="hidden" name="lager" value="ns" />
        ...
        <div className="flex items-end">
          ...
          >
            {pending ? "Wird importiert…" : "Verkäufe importieren (NS-Lager)"}
          </button>
        </div>
      </form>
      ...
      {state.ok && state.imported > 0 && !undoState.done && (
        <div className="flex items-center justify-between rounded-lg border border-green-200 bg-green-50 px-4 py-3">
          <span className="font-mono text-sm text-green-700">
            ✓ {state.imported} Verkauf{state.imported !== 1 ? "e" : ""} aus NS-Lager gebucht.
          </span>
          {showUndo && (
            <form action={undoAction}>
              <input type="hidden" name="ids" value={state.saleIds.join(",")} />
              <button
                type="submit"
      ...
      {undoState.done && (
        <div className="rounded-lg border border-grey-border bg-grey-light px-4 py-3 font-mono text-sm text-grey-dark">
          ✓ {undoState.count} Verkäufe rückgängig gemacht, NS-Bestand wiederhergestellt.
        </div>
      )}
