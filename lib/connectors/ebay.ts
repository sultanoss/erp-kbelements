...
export type EbayPriceEntry = { marketplaceSku: string; title: string | null; price: number };

async function fetchEbayPricesWithCreds(creds: EbayCreds): Promise<EbayPriceEntry[]> {
  const token = await getAccessToken(creds);
  const entries: EbayPriceEntry[] = [];
  let page = 1;
...
<GetMyeBaySelling xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <OutputSelector>PaginationResult</OutputSelector>
    ...
    for (const match of itemMatches) {
      ...
      const sku = block.match(/<SKU>([^<]+)<\/SKU>/)?.[1] ?? "";
      const title = block.match(/<Title>([^<]+)<\/Title>/)?.[1] ?? null;
      const priceStr = block.match(/<StartPrice[^>]*>([^<]+)<\/StartPrice>/)?.[1];
      const price = priceStr != null ? parseFloat(priceStr) : NaN;
      const marketplaceSku = sku || itemId;
      if (marketplaceSku && !isNaN(price)) {
        entries.push({ marketplaceSku, title, price });
      }
      count++;
    }
...
export async function fetchEbayPrices(account: "main" | "outlet" = "main"): Promise<EbayPriceEntry[]> {
  return fetchEbayPricesWithCreds(account === "main" ? mainCreds() : outletCreds());
}
