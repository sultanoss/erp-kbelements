...
export type OttoPriceEntry = { marketplaceSku: string; title: string | null; price: number };

export async function fetchOttoPrices(): Promise<OttoPriceEntry[]> {
  const token = await getToken("products");
  const entries: OttoPriceEntry[] = [];
  let nextUrl: string | null = `${OTTO_PRODUCTS_URL}?limit=100`;
  ...
  while (nextUrl) {
    ...
    if (!res.ok) {
      const text = await res.text();
      console.warn(`Otto Products-Fehler ${res.status}: ${text.slice(0, 200)}`);
      break;
    }
    ...
    const data = await res.json() as {
      resources?: Array<{
        ...
        variants?: Array<{
          sku?: string;
          pricing?: { standardPrice?: { amount?: number } };
          salesPrice?: { amount?: number };
        }>;
        salesPrice?: { amount?: number };
        pricing?: { standardPrice?: { amount?: number } };
      }>;
    ...
    for (const r of data.resources ?? []) {
      ...
      if (variants.length > 0) {
        for (const v of variants) {
          const price = v.pricing?.standardPrice?.amount ?? v.salesPrice?.amount;
          const sku = v.sku ?? r.sku;
          if (sku && price != null) {
            entries.push({ marketplaceSku: sku, title: r.productTitle ?? r.title ?? null, price });
          }
        }
      } else {
        const price = r.pricing?.standardPrice?.amount ?? r.salesPrice?.amount;
        const sku = r.sku;
        if (sku && price != null) {
          entries.push({ marketplaceSku: sku, title: r.productTitle ?? r.title ?? null, price });
        }
